...
function santasNaughtyList(persons, badHabit) {
    ...
    for (obj of persons) {
        for (habit of obj.habits) {
            if (habit === badHabit) {
                naughtyList.push(obj.firstName + " " + obj.lastName)
            }
        }
    }
    return naughtyList
...
function santasNaughtyListFunctional(persons, badHabit) {
    const naughtList = []
    persons.forEach(person => { // person.forEach get a list of the people 
        if (person.habits.includes(badHabit)){ // person.habits.includes gets the habits of each person and sees if it includes the words from badhabit
