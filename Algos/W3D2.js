...
function findObjects(criteria, collection) {
    const filteredArr = []
    let flag = false
    for (const eachItem in collection) {
        const user = collection[eachItem]
        for (const eachCrit in criteria) {
            if (user.hasOwnProperty(eachCrit) && user[eachCrit] === criteria[eachCrit]) {
                flag = true
            } else {
                flag = false
            }
...
function findObjectsFunctional(criteria, collection) {
    for (prop in criteria) {
        collection.filter(filtered => filtered[prop] === collection[prop])
    }
    return collection
}

// console.log(findObjectsFunctional(searchCriteria1, users));
// console.log(findObjectsFunctional(searchCriteria2, users));
