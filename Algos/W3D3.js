...
function filterByKeyStartsWith(items, searchFor, searchBy) {
    ...
    for (eachItem in items){
        if (items[eachItem].hasOwnProperty(searchBy)){
            if (items[eachItem][searchBy].startsWith(searchFor) || items[eachItem][searchBy].toLowerCase().startsWith(searchFor)){
                filteredArr.push(items[eachItem]);
            }
        }
    }
    return filteredArr
